...
function subscribeBook(db){
  ...
  db.doc("league/highlow").onSnapshot(function(snap){
    setHighlow(snap.exists?snap.data():null);   // read-only; narrowed to the season shown
    touch();
  ...
  db.doc("league/players").onSnapshot(function(snap){
    setPlayers(snap.exists?snap.data():null);   // read-only; each player's season so far, for the Draft board
    touch();
  },function(){ /* the board just draws without ranks */ });
  ...
  db.doc("league/draft").onSnapshot(function(snap){
    ...
    touch();
  },function(){ /* the Draft board says it's waiting on a refresh */ });
